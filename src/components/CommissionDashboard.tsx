...
export function CommissionDashboard() {
  const [input, setInput] = useState<CommissionInput>(initialInput);
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 pb-8 font-sans">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-20 shadow-xs">
        <div className="max-w-[1920px] w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 2xl:px-12 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {/* Vodafone Logo */}
            <div className="flex-shrink-0 transition-transform hover:scale-105">
              <VodafoneLogo className="w-9 h-9" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="bg-red-50 text-[#E60000] border border-red-200 text-[10px] font-bold px-2 py-0.5 rounded-full">
                  Vodafone
                </span>
              </div>
              <div className="flex items-center gap-1 text-[11px] sm:text-xs text-slate-500 font-medium">
                ...
                <Heart className="w-3.5 h-3.5 text-[#E60000] fill-[#E60000] inline-block shrink-0 ml-0.5" />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              className="no-print flex items-center gap-1.5 text-xs font-semibold bg-[#E60000] hover:bg-[#CC0000] active:bg-[#B30000] text-white transition-colors px-3.5 py-2 rounded-lg shadow-xs hover:shadow-sm disabled:opacity-70"
            ...
            <button
              onClick={handleReset}
              className="no-print flex items-center gap-1.5 text-xs font-semibold text-slate-700 hover:text-[#E60000] hover:bg-red-50 transition-colors px-3 py-2 rounded-lg border border-slate-200 hover:border-red-200"
      ...
      {/* Main Content Area: Expanded Desktop Width */}
      <main className="max-w-[1920px] w-full mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 2xl:px-12 mt-6 space-y-6">
        
        {/* Full Printable/Exportable Report Container */}
        <div id="printable-report" className="w-full space-y-6">
            ...
            <div className="commission-summary-card lg:col-span-7 bg-gradient-to-br from-[#E60000] via-[#CC0000] to-[#990000] rounded-2xl shadow-md p-4 sm:p-5 lg:p-6 text-white flex flex-col justify-between relative overflow-hidden transition-all">
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  ...
                  <div className="flex items-center gap-1.5 text-xs font-medium bg-black/25 px-2.5 py-1 rounded-md backdrop-blur-xs">
                ...
                <div className="flex items-baseline gap-3 my-2 sm:my-3">
                  ...
                  <div className="text-sm text-white/80 font-medium">
                    {result.overall.achievement !== null && result.overall.achievement >= 100 ? (
                      <span className="bg-white/20 text-white font-bold px-2 py-0.5 rounded">Target Met & Exceeded</span>
                    ) : (
              ...
              <div className="relative z-10 pt-3 sm:pt-4 mt-3 sm:mt-4 border-t border-white/20 grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-2.5 text-xs">
                ...
                <div className="bg-black/20 hover:bg-black/30 rounded-xl p-2 sm:p-2.5 border border-white/15 shadow-xs transition-colors backdrop-blur-xs">
                  ...
                  <div className="text-base font-black mt-1 text-white">{formatPercentage(result.voice.contribution)}</div>
                </div>
                ...
                <div className="bg-black/20 hover:bg-black/30 rounded-xl p-2 sm:p-2.5 border border-white/15 shadow-xs transition-colors backdrop-blur-xs">
                  ...
                  <div className="text-base font-black mt-1 text-white">{formatPercentage(result.enterprise.totalContribution)}</div>
                </div>
                ...
                <div className="bg-black/20 hover:bg-black/30 rounded-xl p-2 sm:p-2.5 border border-white/15 shadow-xs transition-colors backdrop-blur-xs">
                  ...
                  <div className="text-base font-black mt-1 text-white">{formatPercentage(result.terminal.contribution)}</div>
                </div>
                ...
                <div className="bg-black/20 hover:bg-black/30 rounded-xl p-2 sm:p-2.5 border border-white/15 shadow-xs transition-colors backdrop-blur-xs">
                  ...
                  <div className="text-base font-black mt-1 text-white">{formatPercentage(result.fixed.totalContribution)}</div>
                </div>
              </div>
